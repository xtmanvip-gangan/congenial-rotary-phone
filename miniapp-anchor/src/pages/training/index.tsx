...
export default function TrainingPage() {
  ...
  const loadDiy = useCallback(async (force = false) => {
    try {
      const page = await fetchDiyPage('training', { force })
      ...
      console.warn('[Training] DIY 加载失败', e)
    }
  }, [])
    ...
    async (options?: { pullDown?: boolean }) => {
      ...
      try {
        ...
        if (
          ...
          useSessionStore.getState().authLoading
        ) {
  ...
  const diyNav = resolveDiyNav(diySchema?.pageStyle, 'training', '学习中心')
