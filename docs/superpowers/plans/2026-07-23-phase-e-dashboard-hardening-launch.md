...
- [x] **Step 3: Add minimal schema and service**
...
- [x] **Step 4: Generate client and verify**
...
- [x] **Step 1: Write failing role-scope tests**
...
- [x] **Step 3: Implement role-aware aggregation**
...
- [x] **Step 1: Write failing monitoring tests**
...
- [x] **Step 3: Route jobs through JobRunService**
...
- [x] **Step 3: Implement incident lifecycle**
...
- [x] **Step 1: Add typed API contracts and render tests through TypeScript**

定义统一看板、任务运行和异常对象；页面只消费后端已限定角色的数据，不在前端重新拼接全量业务记录。

- [x] **Step 2: Build role workspaces**

审核老师看到激活漏斗；运营看到岗前、首播、缺席补学、反馈和礼物待办；培训人员看到课程执行、参会、问题和异常；超管看到跨模块总览。

- [x] **Step 3: Build operations center**
...
- [x] **Step 4: Run focused and full security tests**
...
- [x] **Step 1: Write failing cleanup preview tests**
...
- [x] **Step 1: Validate database schema**
...
- [x] **Step 3: Verify Web and miniapp**

Run: `npm run build && npm --prefix miniapp-anchor run build:weapp`
Expected: PASS。

- [x] **Step 4: Verify repository scope**
...
- 已增加审核、运营、培训老师、培训管理员和超级管理员的后端角色看板；运营指标由后端固定使用当前账号 ID 限定范围。
- 已增加任务运行表、任务幂等执行、运行计数和运维查询；一小时提醒与失败通知重试已接入。
- 已增加企微和腾讯会议异常归并、错误脱敏、恢复和人工关闭审计。
- 已增加 Web 角色工作台、超管综合指标和“任务与异常”运维中心。
- 已增加超管密码登录失败限流、生产 CORS 白名单、安全响应头、请求体限制和 Excel 扩展名/MIME/ZIP 签名联合校验。
- 已增加测试数据清理只读预览、PostgreSQL 备份/恢复脚本、上线预检和回退演练手册；没有执行数据删除、数据库恢复或生产迁移。
- Prisma schema 使用不连接数据库的占位 URL 静态校验通过。
- API 类型检查、22 个测试文件共 62 个测试、API 构建和生产依赖审计（0 个漏洞）通过。
- Web 生产构建和主播小程序构建通过；小程序构建保留现有 Tailwind `content` 为空警告，不影响本次编译成功。
- 本阶段没有调用真实企业微信、腾讯会议或生产数据库，真实联调与恢复演练仍需在部署环境按运行手册执行。
