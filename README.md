...
- （可选）腾讯会议开放平台配置：当前默认不走 API 建会/同步；会议号与链接在场次页手工回填
...
- 员工多角色和登录入口隔离。
- 审核老师创建、提醒和跟进主播激活任务。
- 主播本人通过企微身份一键开通唯一档案（审核预填资料，主播只读确认）。
- 审核老师分配固定运营，运营确认或驳回；主播不可自选运营。
- 固定运营归属和归属历史数据基础。
...
- 培训场次在系统内自建发布；腾讯会议在腾讯侧人工创建后，回填会议号与入会链接（可后补，不依赖会议 API）。
- 参会以腾讯导出明细/观看时长导入为主路径；按腾讯表头解析，观看时长达计划时长80%自动记为已学，同名冲突人工确认。
